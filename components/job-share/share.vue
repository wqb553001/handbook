	...
	export default {
		...
		watch: {
			shareData(newVal) {
				...
				this.image = newVal.headImgPath
				this.title = newVal.username
				this.href = `${process.env.UNI_BASE_URL || ''}/pages/job/user/user_detail?detailId=${newVal.userId}`
				this.detailId = newVal.userId
			}
		},
		data() {
			return {
				...
				href: `${process.env.UNI_BASE_URL || ''}/pages/job/user/user_detail?detailId=${this.detailId}`, // 用访问链接
				image: this.image || '', // 分享图片
